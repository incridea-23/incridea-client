  ...
  return (
    <Transition appear show={showModal} as={Fragment}>
      <Dialog as="div" className="relative z-10" onClose={onClose}>
        ...
        <div className={`fixed inset-0 overflow-y-auto ${size==='small' && 'w-72 mx-auto'}`}>
          <div className="flex min-h-full items-center justify-center text-center py-5 md:py-7">
            <Transition.Child
              ...
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-gray-700/70 text-gray-100 backdrop-blur-xl text-left align-middle shadow-xl transition-all">
                <Dialog.Title
                  as="div"
                  className={`flex justify-between items-center md:p-6 p-5 ${size === 'small' && 'md:pb-2'}`}>
                  <h3 className={`text-lg font-medium leading-6 text-white ${size ==='small' && 'text-center'}`}>
                    {title}
                  </h3>
                  {size !== 'small' && <button
