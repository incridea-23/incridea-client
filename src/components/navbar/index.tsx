    ...
    <nav
      ...
      <div className="max-w-5xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex order-2 lg:order-1 items-center space-x-2">
            <Image
              className="w-24"
          ...
          <div className="hidden space-x-6 lg:order-2 text-gray-900 lg:flex">
            {links.map((link) => (
              <Link
                ...
                key={link.url}
                href={link.url}>
                <CharacterAnimation text={link.label} textStyle="text-lg font-medium" />
              </Link>
            ))}
          </div>
  ...
  return (
    <div className={`flex space-x-2  lg:px-0 ${className}`}>
      {status === "authenticated" && (
        router.pathname === "/profile" && user?.role !== 'USER' && user?.role !== 'PARTICIPANT' ? (
