...
const SignIn: NextPage = () => {
  ...
  return (
    <div className="min-h-screen min-w-screen bg-cover bg-gradient-to-bl from-[#46aacf]  via-[#075985] to-[#2d6aa6]">
      <div className={`hidden md:block w-[100vh] fixed top-1/2 -translate-y-1/2 transition-transform duration-500 -rotate-90 left-[47%] ${whichForm === 'signUp' ? '-translate-x-[110%]' : '-translate-x-1/2'}`}>
        <img src={"/wave.svg"} className="w-[100vh]" />
      </div>
      <div className={`hidden md:block w-[100vh]  fixed top-1/2 -translate-y-1/2 transition-transform duration-500 rotate-90 right-[47%] ${whichForm !== 'signUp' ? 'translate-x-[110%]' : 'translate-x-1/2'}`}>
        <img src={"/wave.svg"} className="w-[100vh]" />
      </div>
      <div className="flex w-screen h-screen">
        <div
          ...
          {whichForm === "signUp" ? (
            <div className={`${titleFont.className} pl-12`}>Signup text/mascot</div>
          ) : (
            <div className={`${titleFont.className} pr-12`}>Signin text/mascot</div>
          )}
        </div>
        <div
